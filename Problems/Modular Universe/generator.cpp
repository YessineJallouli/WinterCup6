...
{
    auto [seed,queries, args] = parse_arguments(argc,argv);
    ...
    IntegerDistributionManager integerManager;
    QueryDistributionManager queryManager;
    ...
    queryManager.add<UniformQueryDistribution>("uniform",integerManager);
    queryManager.add<UniformDivisorQueryDistribution>("divisors",factoriser,integerManager);
    std::span params(args);
    auto query_dist = setup_distribution(params,queryManager);
    std::cout << queries << '\n';
    for(int i=0;i<queries;i++)
    {
        auto Q=query_dist->sample();
        std::cout << Q.n << ' ' << Q.m << ' ' << Q.x << ' ' << Q.y << ' ' << Q.d << '\n';
    }
    return 0;
}
